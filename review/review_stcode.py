...
#获取历史交易日
trade_cal = pro.trade_cal(is_open='1',fileds='cal_date',start_date=get_day_time(60), end_date=get_day_time(10))
...
#通过时间段获取数据方法
def getRecordsByDaterange(start_date,end_date):
    query = { "$and": [{"trade_date":{"$gte":start_date}},{"trade_date":{"$lte":end_date}}]} 
    #query = { "$and": [{"trade_date":{"$gte":get_day_time(60)}},{"trade_date":{"$lte":get_day_time(1)}}]}
    rs_daterange = mycollection.find(query)
    #将查询结果转换为Df
            ...
            if (row[1]['ma20'][0]>row[1]['ma20'][1]>row[1]['ma20'][2]>row[1]['ma20'][3]>row[1]['ma20'][4]):
                if (row[1]['ma30'][0]>row[1]['ma30'][1]>row[1]['ma30'][2]>row[1]['ma30'][3]>row[1]['ma30'][4]):
                    if(row[1]['close']>row[1]['ma5'][0] and row[1]['preclose']<row[1]['ma5'][0]):
                        ...
                        st_code=row[1]['ts_code']                       
                        stratege_result[st_code] = row[1]['close']
                        n=n+1
#print (stratege_result)

#获取收益日行情数据
d_df=getRecordsByDaterange(D_DATE,D_DATE)
#print(d_df)

#收益率计算
avg_benfit=[]
for item in  d_df.iterrows():
    item_close=item[1]['close']
    item_stcode = item[1]['ts_code']
    item_tradedate = item[1]['trade_date']
    if (item_stcode in stratege_result):
        benfit = (item_close - stratege_result[item_stcode])/stratege_result[item_stcode]
        print(item_stcode,benfit,item_tradedate,D_DATE)
        avg_benfit.append(benfit)
print (np.mean(avg_benfit))
