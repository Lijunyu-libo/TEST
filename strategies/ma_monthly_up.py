...
dailycollection=mydb["dailytest"]
...
query = { "$and": [{"trade_date":{"$gte":get_day_time(480)}},{"trade_date":{"$lte":get_day_time(0)}}]}
rs_daterange = mycollection.find(query)
#将查询结果转换为Df
df = pd.DataFrame(list(rs_daterange))
#获取当天收盘价集合
todayquery = { "$and": [{"trade_date":{"$gte":get_day_time(0)}},{"trade_date":{"$lte":get_day_time(0)}}]}
rs_today = dailycollection.find(todayquery)
df_today = pd.DataFrame(list(rs_today))
#计算前日线均值MA
...
#封装df_ma
df_ma = pd.DataFrame(columns=('ts_code','close','preclose', 'ma5', 'ma10','ma20'))
for name,group in df.groupby(['ts_code']):
    close_arr=group['close'].tolist()
    preclose_arr=group['pre_close'].tolist()
    data_today = df_today[df_today['ts_code']==name] 
    #补充当天收盘价
    close_arr.append(data_today['close'])
    #preclose_arr.append(data_today['pre_close'])
    close_arr.reverse()
    preclose_arr.reverse()
    ma5=ma(close_arr,5)
    ma10=ma(close_arr,10)
    ma20=ma(close_arr,20)
    df_ma = df_ma.append([{'ts_code':name,'close':close_arr[0],'preclose':preclose_arr[0],'ma5':ma5,'ma10':ma10,'ma20':ma20}], ignore_index=False)
#print (df_ma)
'''
...
for row in df_ma.iterrows():
    #5日线反转，10，20连续4日纠缠
    if (row[1]['ma5'][0]>row[1]['ma5'][1] and row[1]['ma5'][2]>row[1]['ma5'][1] and row[1]['ma5'][0]<row[1]['ma5'][2]):
